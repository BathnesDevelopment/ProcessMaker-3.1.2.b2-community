...
(function () {
    var PanelView = Backbone.View.extend({
        ...
        printForm: function () {
            ...
            formMode = forms && _.isArray(forms) && forms.length > 0 ? forms[0].model.get("mode") : formMode;

            if ((project && project.isPreview) || formMode === "disabled" || formMode === "view") {
                window.print();
            } else {
...
(function () {
    var FormPanel = Backbone.View.extend({
        ...
        initialize: function (options) {
            ...
            this.onChangeCallback = new Function();
            ...
            this.validDependentFields = ["dropdown", "suggest", "text", "textarea", "label", "radio", "checkgroup"];
        ...
        createDependencies: function () {
            ...
            for (i = 0; i < fields.length; i += 1) {
                ...
                if (fields[i].get("type") === "checkgroup") {
                    continue;
                }
...
(function () {
    var FieldView = Backbone.View.extend({
        ...
        runDependentIfExist: function () {
            if (this.isDependent()) {
                this.model.onDependentHandler();
            }
            return this;
        },
...
(function () {
    var GridView = PMDynaform.view.Field.extend({
        ...
        updateMultipleFileCell: function (cell, i, j) {
            ...
            cell.model.set('nameToPostControl', this.createNameCell(index, cell.model.get('columnName')));
            cell.model.set('nameToPostLabelControl', this.createNameToLabelCell(index, cell.model.get('columnName')));
        ...
        getColumnHeader: function (index) {
            if (this._gridHeader) {
                return this._$gridHeader.find('> *').eq(index);
            }
            return null;
        },
        hideColumn: function (col) {
    ...
    var RadioView = PMDynaform.view.Field.extend({
        ...
        updateDom: function (option) {
            ...
            if (selectedHTMLOption && selectedHTMLOption.length) {
                selectedHTMLOption.attr('checked', true);
        ...
        getControl: function () {
            var htmlControl = this.$el.find("input[type='radio']");
            return htmlControl;
        }
    });
...
(function () {
    var TextareaView = PMDynaform.view.Field.extend({
        ...
        getControl: function () {
            var htmlControl = this.$el.find("textarea");
            return htmlControl;
        }
    });
...
(function () {
    var TextView = PMDynaform.view.Field.extend({
        ...
        updateDataModel: function () {
            ...
            this.model.attributes.value = this.tagControl.val();
            this.model.attributes.text = this.tagControl.val();
            this.model.set("data", data);
            return this;
        },
    ...
    var File = PMDynaform.view.Field.extend({
        ...
        render: function () {
            var that = this,
                ...
                link,
                i,
                label = that.model.get("data")["label"];
...
(function () {
    ...
    var SuggestView = PMDynaform.view.Field.extend({
        ...
        events: {
            "click .list-group-item": "continueDependentFields",
            "keyup input": "generateOptionsHandler",
            "change input": "eventListener",
            "keydown input": "refreshBinding",
            "blur input": "_abortRequest"
        },
        stamp: null,
        timeToWait: 500,
        timeoutHandler: null,
            ...
            $(this.containerList).on('keydown', function (e) {
                ...
                if (that.stackItems.length > 0) {
                    if (code === 38 && that.stackRow > 1) { // UP
                        ...
                        currentTarget = that.containerList.find(".pmdynaform-suggest-list-keyboard");
                        label = currentTarget.find("a").data().label;
        ...
        _attachSuggestGlobalEvents: function () {
            if (this.containerList) {
                $(document).on("click." + this.cid, $.proxy(this.hideSuggest, this));
            }
        },
        ...
        makeNewListItems: function (localData, val, event) {
            var that = this;
            if (this.itHasSqlQuery()) {
                this.executeSuggestQuery(function (data) {
        ...
        itHasSqlQuery: function () {
            return this.model.itHasSqlQuery();
        },
        /**
        ...
        filterLocalOptions: function (value, maxItems, event) {
            ...
            $.grep(that.elements, function (options, index) {
                ...
                if ((itemLabel.toLowerCase().indexOf(value.toLowerCase()) !== -1) && count < maxItems) {
                    that.updatingItemsList(options, event);
                    data.push(options);
        ...
        refreshSuggestList: function (data, value, maxItems, event) {
            ...
            this.stackItems.click(function (e) {
        ...
        continueDependentFields: function (e) {
            ...
            this.clicked = true;
            this.keyPressed = false;
            label = $(e.currentTarget).find("a").data().label;
...
(function () {
    var Label = PMDynaform.view.Field.extend({
        ...
        initialize: function () {
            ...
            this.model.on("change:data", this.updateViewControls, this);
        },
        ...
        onChange: function (event) {
            ...
            if (data["label"] !== undefined && data["label"] !== null) {
                this.setFullOptions(data["label"]);
            }
            return this;
        },
        render: function () {
...
(function () {
    var DatetimeView = PMDynaform.view.Field.extend({
        ...
        render: function () {
            ...
            value = this.model.get("value");
            if (value) {
                control.data("DateTimePicker").date(moment(value));
            }
        ...
        formatData: function (control) {
            var valueDB,
                data,
                originalFormat = this.model.get("format"),
            ...
            if (!valueDB) {
                ...
                valueDB = moment(data.date, originalFormat).format(formatDB);
            }
            return valueDB;
        },
        setValue: function (value) {
        ...
        recalculateWidgetPostion: function () {
            ...
            appendTo = this.getParentElementForFloatingElements();

            this.updateArrowsToRTL(widget);

            rect = text.get(0).getBoundingClientRect();

            if (isMobile && appendTo === document.body) {
                mobileRect = text.offset();
                mobileRect.right = mobileRect.left + text.outerWidth();
                mobileRect.bottom = mobileRect.top + text.outerHeight();
            } else {
                mobileRect = rect;
            }

            availableSpace = $(window).outerHeight() - rect.bottom;

            // Let's check if the calendar should be shown up or down the control
            if (availableSpace < rect.top) {
                y = (isMobile ? mobileRect.top : rect.top) - widgetHeight;
            } else {
                y = isMobile ? mobileRect.bottom : rect.bottom;
            }

            availableSpace = $(window).outerWidth() - rect.left;

            if (widgetWidth <= availableSpace || widgetWidth > rect.right + icon.outerWidth()) {
                x = rect.left;
            } else {
                x = rect.right + icon.outerWidth() - widgetWidth;
            }

            if (appendTo !== document.body) {
                x += appendTo.scrollLeft;
                y += appendTo.scrollTop;
            }
            if ( widget.length ) {
                appendTo.appendChild(widget.get(0));
    ...
    var FileMobile = PMDynaform.view.Field.extend({
        ...
        onClickButtonImage: function (event) {
		 ...
		 cameraEnabled: this.model.get("cameraEnabled")	
            ...
            this.model.get("project").requestManager.getImage(respData);
            return this;
        },
    ...
    var FieldModel = Backbone.Model.extend({
        ...
        mergeOptions: function (remoteOptions) {
            var k,
                ...
                localOpt,
                options;
            if (_.isArray(remoteOptions)) {
                remoteOptions = this.formatResponse(remoteOptions);
            } else {
                remoteOptions = this.get("optionsSql");
            }
            this.set("remoteOptions", remoteOptions);
            this.set("optionsSql", remoteOptions);
            localOpt = this.get("localOptions");
            options = localOpt.concat(remoteOptions);
        ...
        onDependentHandler: function () {
            var remoteOptions,
                type,
                autocompleteInvalidData = ["checkgroup", "radio"];
            type = this.get("mode") === "view" ? this.get("originalType") : this.get("type");
            this.jsonData = this.generateDataDependenField();
            remoteOptions = this.executeQuery();
            this.mergeOptions(remoteOptions);
            if (this.get("options").length) {
                if (!this.get("value") && autocompleteInvalidData.indexOf(type) === -1){
                    this.set("data", {
                        value: this.get("options")[0].value,
                        label: this.get("options")[0].label
                    });
                    this.set("fullOptions", [this.get("data")["label"]]);
                }
            }
            return this;
        },
...
(function () {
    var File = PMDynaform.model.Field.extend({
        ...
        /**
         * Get array of labels from file control
         * @returns {Array}
         */
        getValue: function () {
            var data = this.get("data"),
                label = [];
            label = data && data["label"] ? data["label"] : label;
            return label;
        }
    });
...
(function () {
    var FileMobile = PMDynaform.model.Field.extend({
        ...
        validate: function () {
            ...
            if (PMDynaform.core.ProjectMobile) {
                if (data && _.isBoolean(this.get("required")) && this.get("required")) {
                    if (_.isArray(data.value) && data.value.length > 0) {
                        isValid = true;
                    }
                }
    ...
    var MultipleFileModel = PMDynaform.model.Field.extend({
        ...
        initialize: function (options) {
            ...
            this.set("validator", new Backbone.Model({
                message: this.get("messageValidations")
            }));
            return this;
        },
        /**
        ...
        addFileModel: function (file) {
            ...
            fileSize = fileModel.validateFileSize();
            if (!fileSize) {
...
(function () {
    var MultipleFileView = PMDynaform.view.Field.extend({
        ...
        setData: function (data) {
            var value,
                box = this.$el.find(".pmdynaform-file-box"),
                index;
            try {
                value = _.isString(data.value) ? JSON.parse(data.value) : data.value;
        ...
        createDelHiddens: function (model) {
            ...
            this.model.get('toRemoveArray').push(model);
            return this;
        },
        /**
        ...
        createNameforHidden: function (prop, model) {
            ...
            if (this.model.get("group") === "grid") {
                ...
                indexCol = "[" + index.col + "]";
                name = varName + "[" + this.parent.model.get("name") + "]" + indexCol + colName + "[" + index.row + "]" + propName;
            } else {
    ...
    var FileModel = Backbone.Model.extend({
        ...
        prepareDataToUpload: function (file) {
            ...
            arrayResp.push({
                ...
                "docUid": this.get("parent").get("inp_doc_uid"),
                "appDocType": "INPUT"
            });
        ...
        /**
         * Return a object to render
         * @returns {{ext: *, icon: *, name: *}|*}
         */
        getDataFile: function () {
            var obj, ext, icon, name,
                file = _.extend({}, this.get("file"));

            ext = this.parseExtension(file.name);
            icon = this.get("types")[ext];
            name = this.parseName(file.name);
            obj = {
                "ext": ext,
                "icon": icon ? icon : this.get("types")["default"],
                "name": name
            };
            return obj;
        },
        ...
        getDataFileLink: function () {
            ...
            obj = {
                ...
                "icon": icon ? icon : "txt",
                "name": name,
                "href": this.get("project").webServiceManager.showDocument(data)
            };
            return obj;
        },
        /**
        ...
        /**
         * Validate the file size in file
         * @returns {*}
         */
        validateFileSize: function () {
            var limit, unit, size, total, resp;
            limit = this.get("size");
            unit = this.get("sizeUnity");
            size = this.get("file")["size"];
            limit = _.isString(limit) ? parseFloat(limit) : limit;
            total = limit;
            switch (unit) {
                case "MB":
...
(function () {
    var FileView = Backbone.View.extend({
        ...
        templateIcon: _.template($("#tpl-multiplefile-icon").html()),
        templateButtons: _.template($("#tpl-multiplefile-buttons").html()),
        templateDownload: _.template($("#tpl-multiplefile-download").html()),
        col: 12,
        progressBar: null,
        colProgressBar: 10,
        colIcon: 1,
        ...
        messageFail: "Failed",
        events: {
            "click .multiplefile-button-delete": "onClickDelete"
        },
        $buttonDelete: null,
        $buttonDownload: null,
        initialize: function (options) {
            this.$hiddens = [];
            this.progressBar = new PMDynaform.ui.ProgressBar({
                percentage: 0,
                title: this.model.get("file").name,
                col: this.colProgressBar,
                type: "file",
                striped: false,
                animate: false
            });
        ...
        onClickCancel: function (e) {
            ...
            if (xhr && xhr.abort) {
                xhr.abort();
                if (_.isFunction(this.progressBar.setType)) {
         ...
         */
        render: function () {
            switch (this.model.get('parent').get("mode")) {
                case "disabled":
                case "view":
                    this.renderView();
                    break;
        ...
        /**
         * Render mode edit - preogressbar, buttons etc
         * @returns {FileView}
         */
        renderEdit: function () {
            var obj = this.model.getDataFile(),
                container,
                enableButtonDownload = this.model.get('fromWizard') ? false : true,
                newConfig = this.calculateColsView({
                    mode: "edit",
                    buttons: enableButtonDownload ? 2 : 1
                });

            this.setElement(this.template({
                col: this.col
            }));
            container = this.$el.find(".multiplefile-container");
            obj["col"] = newConfig.colIcon;
            container.append(this.templateIcon(obj));

            if (this.model.get('fromWizard')) {
                this.progressBar.render();
                container.append(this.progressBar.$el);
            } else {
                container.append(this.templateDownload({
                    title: obj.name,
                    col: newConfig.colName,
                    href: this.model.getLinkDownload()
                }));
            }
            container.append(this.templateButtons({
                col: newConfig.colButtons,
                colButton: newConfig.colButton,
                href: this.model.getLinkDownload()
            }));
            this.$buttonDownload = this.$el.find(".multiplefile-button-download");
            this.$buttonDelete = this.$el.find(".multiplefile-button-delete");

            if (!enableButtonDownload) {
         ...
         * Calculate the cols
         * @param config
         * @returns {{colIcon: number, colButtons: number, colName: number, colButton: number}}
         */
        calculateColsView: function (config) {
            var configNew = {
                colIcon: 1,
                colButtons: 1,
                colName: 11,
            ...
            if (config.mode === "edit") {
                configNew.colButtons = config.buttons === 1 ? configNew.colButtons : config.buttons + 1;
                configNew.colName = config.buttons === 1 ? configNew.colName : configNew.colName - config.buttons - 1;
                configNew.colButton = (configNew.colButton / config.buttons) - 1;
            } else {
                configNew.colButton = configNew.colButton - 1;
                configNew.colName = configNew.colName - 1;
            }
            return configNew;
        },
        /**
        ...
        renderView: function () {
            ...
            container.find(".multiplefile-title").tooltip({
                title: obj.name
            });
            this.$buttonDownload = this.$el.find(".multiplefile-button-download");
            this.$buttonDelete = this.$el.find(".multiplefile-button-delete");
        ...
        updatePercentage: function () {
            var per = this.model.get("percentage");
            per = Math.floor(per);
            if (_.isFunction(this.progressBar.setPercentage)) {
                this.progressBar.setPercentage(per);
                this.progressBar.setMessage(per + "%");
            }
            return this;
        },

        setPercentageToProgressBar: function (value) {
            if (_.isFunction(this.progressBar.setPercentage)) {
                this.progressBar.setPercentage(value);
            }
            return this;
        },
        ...
        /**
         * FinishLoad the file & update the view
         * @returns {FileView}
         */
        finishLoad: function () {
            if (this.progressBar) {
                this.progressBar.disableAnimation();
                this.progressBar.setStriped(false);
            }
            return this;
        },
        /**
        ...
        onClickDelete: function () {
            ...
            if (this.model.get("parent").get("mode") === "edit" && parentView) {
                ...
                this.model.get('parent').createRemovedHiddens(this.model);
            }
            return this;
        }
    });
...
(function () {
    ...
    var UploadModalModel = PMDynaform.file.MultipleFileModel.extend({
         ...
         */
        addFileModel: function (file) {
            var files = this.get('parent').get('fileCollection'),
                fileSize,
                fileModel;
            ...
            fileModel = new PMDynaform.file.FileModel({
                ...
                project: this.get('parent').get('project'),
                form: this.get('parent').get('form'),
                size: this.get('parent').get('size'),
                sizeUnity: this.get('parent').get('sizeUnity')
            });
            fileSize = fileModel.validateFileSize();
            if (!fileSize) {
                fileModel.set("isValid", fileSize);
...
(function () {
    var UploadModal = Backbone.View.extend({
        ...
        renderForm: function (collection) {
            ...
            box = this.modal.find(".pmdynaform-file-box");
